...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, isHttpPostBinding, callback) {
  ...
  .then(() => {
    ...
    if (this.options.assertionConsumingServiceIndex != null) {
      request['samlp:AuthnRequest']['@AssertionConsumingServiceIndex'] = this.options.assertionConsumingServiceIndex;
      delete request['samlp:AuthnRequest']['@ProtocolBinding'];
      delete request['samlp:AuthnRequest']['@AssertionConsumerServiceURL'];      
    }
    ...
    console.log('XML Request', stringRequest);
...
SAML.prototype.generateServiceProviderMetadata = function( decryptionCert, signingCert ) {
  ...
  const providerMetadata = xmlbuilder.create(metadata).end({ pretty: true, indent: '  ', newline: '\n' });
